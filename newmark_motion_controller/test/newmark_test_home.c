/* newmark_test_home.c
** $Header: /home/cjm/cvs/frodospec/newmark_motion_controller/test/newmark_test_home.c,v 1.3 2011-01-05 14:17:04 cjm Exp $
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * This program tests the Newmark motion controller home command.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: newmark_test_home.c,v 1.3 2011-01-05 14:17:04 cjm Exp $";
/**
 * Variable holding which type of device we are using to communicate with the PLC.
 * @see http://ltdevsrv.livjm.ac.uk/~dev/arcom_ess/cdocs/arcom_ess_interface.html#ARCOM_ESS_INTERFACE_DEVICE_ID
 */
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Test Newmark Home command.\n");
	/* initialise logging */
	...
	if(!Arcom_ESS_Interface_Open("newmark_test_home",NULL,Device_Id,Device_Name,Port_Number,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	/* home motion controller */
	if(!Newmark_Command_Home("newmark_test_home",NULL,handle))
	{
		Newmark_Error();
		Arcom_ESS_Error();
		return 5;
	...
	/* close interface */
	if(!Arcom_ESS_Interface_Close("newmark_test_home",NULL,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	...
	fprintf(stdout,"Newmark Test Home:Finished Test ...\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if(strcmp(argv[i],"-baud_rate")==0)
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Newmark Test Home :Parse_Arguments:"
						"Illegal baud rate : %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
		...
		{
			fprintf(stderr,"Newmark Test Home:Parse_Arguments:argument '%s' not recognized.\n",argv[i]);
			return FALSE;
		}			
	}
	return TRUE;
...
static void Help(void)
{
	fprintf(stdout,"Newmark Test Home:Help.\n");
	fprintf(stdout,"Newmark Test Home tests the homing of the Newmark motion controller.\n");
	fprintf(stdout,"newmark_test_home [-serial_device <filename>][-socket_device <address> <port>]\n");
