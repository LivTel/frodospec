/* newmark_test_move_relative.c
** $Header: /home/cjm/cvs/frodospec/newmark_motion_controller/test/newmark_test_move_relative.c,v 1.3 2011-01-05 14:17:04 cjm Exp $
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * This program tests the Newmark motion controller MOVR command - move relative.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: newmark_test_move_relative.c,v 1.3 2011-01-05 14:17:04 cjm Exp $";
/**
...
/**
 * The relative position offset of the slide.
 */
static double Position_Offset = 0;
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Newmark Test Move Relative command.\n");
	/* initialise logging */
	...
	if(!Arcom_ESS_Interface_Open("newmark_test_move_relative",NULL,Device_Id,Device_Name,Port_Number,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	/* offset motion controller position */
	if(!Newmark_Command_Move_Relative("newmark_test_move_relative",NULL,handle,Position_Offset))
	{
		Newmark_Error();
		Arcom_ESS_Error();
		return 5;
	...
	/* close interface */
	if(!Arcom_ESS_Interface_Close("newmark_test_move_relative",NULL,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	...
	fprintf(stdout,"Newmark Test Relative:Finished Test ...\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if(strcmp(argv[i],"-baud_rate")==0)
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Newmark Test Relative :Parse_Arguments:"
						"Illegal baud rate : %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
		...
		else if((strcmp(argv[i],"-position_offset")==0)||(strcmp(argv[i],"-p")==0))
		{
			if((i+1)<argc)
			{
				retval = sscanf(argv[i+1],"%lf",&Position_Offset);
				if(retval != 1)
				{
					fprintf(stderr,"Newmark Test Relative:Parse_Arguments:"
						"Illegal position offset %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
			}
		...
		{
			fprintf(stderr,"Newmark Test Relative:Parse_Arguments:argument '%s' not recognized.\n",argv[i]);
			return FALSE;
		}			
	}
	return TRUE;
...
static void Help(void)
{
	fprintf(stdout,"Newmark Test Relative:Help.\n");
	fprintf(stdout,"Newmark Test Relative tests offseting the position of the Newmark motion controller.\n");
	fprintf(stdout,"newmark_test_move_relative [-serial_device <filename>][-socket_device <address> <port>]\n");
	fprintf(stdout,"\t[-p[osition_offset] <mm>][-log_level <number>][-help][-baud_rate <B9600|B19200>]\n");
	fprintf(stdout,"\n");
	...
	fprintf(stdout,"\t-position_offset specifies the position offset in mm.\n");
