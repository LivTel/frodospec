/* newmark_test_abort_move.c
** $Header: /home/cjm/cvs/frodospec/newmark_motion_controller/test/newmark_test_abort_move.c,v 1.3 2011-01-05 14:17:04 cjm Exp $
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
 * This program tests the Newmark motion controller '<esc>' command, which aborts any movement.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: newmark_test_abort_move.c,v 1.3 2011-01-05 14:17:04 cjm Exp $";
/**
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Test Newmark Abort Move command.\n");
	/* initialise logging */
	...
	if(!Arcom_ESS_Interface_Open("newmark_test_abort_move",NULL,Device_Id,Device_Name,Port_Number,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	/* abort_move motion controller */
	if(!Newmark_Command_Abort_Move("newmark_test_abort_move",NULL,handle))
	{
		Newmark_Error();
		Arcom_ESS_Error();
		return 5;
	...
	/* close interface */
	if(!Arcom_ESS_Interface_Close("newmark_test_abort_move",NULL,handle))
	{
		Arcom_ESS_Error();
		return 3;
	}
	...
	fprintf(stdout,"Newmark Test Abort Move:Finished Test ...\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		if(strcmp(argv[i],"-baud_rate")==0)
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Newmark Test Abort Move :Parse_Arguments:"
						"Illegal baud rate : %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
		...
		{
			fprintf(stderr,"Newmark Test Abort Move:Parse_Arguments:argument '%s' not recognized.\n",argv[i]);
			return FALSE;
		}			
	}
	return TRUE;
...
static void Help(void)
{
	fprintf(stdout,"Newmark Test Abort Move:Help.\n");
	fprintf(stdout,"Newmark Test Abort Move tests aborting movement of the Newmark motion controller.\n");
	fprintf(stdout,"newmark_test_abort_move [-serial_device <filename>][-socket_device <address> <port>]\n");
