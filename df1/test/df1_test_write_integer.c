/* df1_test_write_integer.c
** $Header: /home/cjm/cvs/frodospec/df1/test/df1_test_write_integer.c,v 1.1 2023-03-21 14:36:52 cjm Exp $
 */
...
/**
 * This program tests writing an integer value to a PLC.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: df1_test_write_integer.c,v 1.1 2023-03-21 14:36:52 cjm Exp $";
/**
...
/**
 * The integer value to write to the PLC address. Initialised to -1 so we can tell if it has not been set.
 */
static int Value = -1;
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Test Writing Integer values to the PLC.\n");
	/* initialise logging */
	...
	if(Value < 0)
	{
		fprintf(stderr,"The integer value was not set.\n");
		return 2;
	}
	/* open the interface */
	...
	/* write integer value */
	word_value = (unsigned short)Value;
	fprintf(stdout,"Test Write Integer:Writing integer value %hu (%d) to PLC Address '%s'.\n",
		word_value,Value,PLC_Address);
	if(!Df1_Write_Integer(handle,SLC,PLC_Address,word_value))
	{
		Df1_Error();
		return 4;
	}
	fprintf(stdout,"Test Write Integer:Written integer value %hu (%d) to PLC Address '%s'.\n",
		word_value,Value,PLC_Address);
	/* close interface */
	...
	fprintf(stdout,"Test Write Integer:Finished Test ...\n");
	return 0;
}
...
static int Parse_Arguments(int argc, char *argv[])
{
	...
	for(i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i],"-value")==0)
		{
			...
			{
				fprintf(stderr,"Test Write Integer:Parse_Arguments:"
					"Value requires an positive integer.\n");
				return FALSE;
			}
		}
...
static void Help(void)
{
	...
	fprintf(stdout,"Test Write Integer tries writing an integer value to a PLC.\n");
	fprintf(stdout,"df1_test_write_integer [-serial_device <filename>][-socket_device <address> <port>]\n");
	...
	fprintf(stdout,"\t-value specifies the value of the integer to write to the PLC address.\n");
