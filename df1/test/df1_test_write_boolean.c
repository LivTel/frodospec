/* df1_test_write_boolean.c
** $Header: /home/cjm/cvs/frodospec/df1/test/df1_test_write_boolean.c,v 1.1 2023-03-21 14:36:52 cjm Exp $
 */
...
/**
 * This program tests writeing a boolean value from a PLC.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: df1_test_write_boolean.c,v 1.1 2023-03-21 14:36:52 cjm Exp $";
/**
...
/**
 * The address of PLC containing the boolean value to query.
 */
static char PLC_Address[256];
/**
 * The boolean value to write to the PLC address. Initialised to -1 so we can tell if it has not been set.
 */
static int Value = -1;
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Test Writing boolean values to the PLC.\n");
	/* initialise logging */
	...
	if(Value < 0)
	{
		fprintf(stderr,"The boolean value was not set.\n");
		return 2;
	}
	/* open the interface */
	...
	/* write boolean value */
	fprintf(stdout,"Test Write Boolean:Writing boolean value %d to PLC Address '%s'.\n",Value,PLC_Address);
	if(!Df1_Write_Boolean(handle,SLC,PLC_Address,Value))
	{
		Df1_Error();
		return 4;
	}
	fprintf(stdout,"Test Write Boolean:Written boolean value %d to PLC Address '%s'.\n",Value,PLC_Address);
	/* close interface */
	...
	fprintf(stdout,"Test Write Boolean:Finished Test ...\n");
	return 0;
}
	...
	for(i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i],"-value")==0)
		{
			if((i+1)<argc)
			{
				...
				{
					fprintf(stderr,"Test Write Boolean:Parse_Arguments:"
						"Illegal Value %s : Should be true/false.\n",argv[i+1]);
					return FALSE;
				}
			...
			{
				fprintf(stderr,"Test Write Boolean:Parse_Arguments:"
					"Value requires an argument: true/false.\n");
				return FALSE;
			}
		}
		...
		{
			fprintf(stderr,"Test Write Boolean:Parse_Arguments:argument '%s' not recognized.\n",argv[i]);
			return FALSE;
		}			
	}
	return TRUE;
...
static void Help(void)
{
	fprintf(stdout,"Test Write Boolean:Help.\n");
	fprintf(stdout,"Test Write Boolean tries writing a boolea value from a PLC.\n");
	fprintf(stdout,"df1_test_write_boolean [-serial_device <filename>][-socket_device <address> <port>]\n");
	fprintf(stdout,"\t[-address <string>][-value <true|false>]\n");
	...
	fprintf(stdout,"\t-address specifies the boolean PLC address to set, of the form B3:0/5.\n");
