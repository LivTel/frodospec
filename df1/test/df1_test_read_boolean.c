/* df1_test_read_boolean.c
** $Header: /home/cjm/cvs/frodospec/df1/test/df1_test_read_boolean.c,v 1.1 2023-03-21 14:36:52 cjm Exp $
 */
...
/**
 * This program tests reading a boolean value from a PLC.
 * @author $Author: cjm $
 ...
 */
static char rcsid[] = "$Id: df1_test_read_boolean.c,v 1.1 2023-03-21 14:36:52 cjm Exp $";
/**
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout,"Test Reading boolean values from PLC.\n");
	/* initialise logging */
	...
	/* read boolean value */
	if(!Df1_Read_Boolean(handle,SLC,PLC_Address,&ivalue))
	{
		Df1_Error();
		return 4;
	}
	fprintf(stdout,"Test Read Boolean:PLC Address '%s' has value %d\n",PLC_Address,ivalue);
	/* close interface */
	...
	fprintf(stdout,"Test Read Boolean:Finished Test ...\n");
	return 0;
}
...
static int Parse_Arguments(int argc, char *argv[])
{
	...
	for(i=1;i<argc;i++)
	{
		if(strcmp(argv[i],"-serial_device")==0)
		{
			...
			{
				fprintf(stderr,"Test Read Boolean:Parse_Arguments:"
		...
		{
			fprintf(stderr,"Test Read Boolean:Parse_Arguments:argument '%s' not recognized.\n",argv[i]);
			return FALSE;
		}			
	}
	return TRUE;
...
static void Help(void)
{
	fprintf(stdout,"Test Read Boolean:Help.\n");
	fprintf(stdout,"Test Read Boolean tries reading a boolea value from a PLC.\n");
	fprintf(stdout,"df1_test_read_boolean [-serial_device <filename>][-socket_device <address> <port>]\n");
